...
contract SpotCoin {
    ...
    //----------------------------------------------------------------------------------------------------------------//
    // 1. SpotCoin
    enum TransactionType { exchange, received, sent, spot_creation, spot_purchase, spot_sale }
    ...
    */
    ...
    function get_user_by_public_key(string memory public_key) public view returns (User memory) {
        ...
        if (check_user_exists_pb(public_key))
            return users[pb_users_map[public_key] - 1];
    ...
    function transfer_money(string memory sender_pub, string memory receiver_pub, int amount) public {
        ...
        if (sender.balance > amount)
        {
            ...
            sender.transaction_ids[sender.transaction_ids.length] = sender.transaction_ids.length;
            ...
            receiver.transaction_ids[receiver.transaction_ids.length] = receiver.transaction_ids.length;
